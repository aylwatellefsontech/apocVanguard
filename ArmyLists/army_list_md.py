#!/usr/bin/env python3
"""Army list JSON ↔ Markdown utilities."""
...
DEFAULT_JSON_GLOB = "Apoc40k-Armies-1st - *.json"
DEFAULT_MD_GLOB = "Apoc40k-Armies-1st - *.md"


def cmd_to_md(paths: list[str]) -> None:
    json_paths = [Path(p) for p in paths] if paths else sorted(ARMY_LISTS_DIR.glob(DEFAULT_JSON_GLOB))
...
def cmd_to_json(paths: list[str], sync_web: bool) -> None:
    md_paths = [Path(p) for p in paths] if paths else sorted(ARMY_LISTS_DIR.glob(DEFAULT_MD_GLOB))
...
def cmd_verify(paths: list[str]) -> None:
    ...
    for json_path in json_paths:
        ...
        normalize_text_fields(original)
        md_text = json_to_markdown(original)
        restored = markdown_to_json(md_text, json_path.with_suffix(".md"))
...
def main():
    parser = argparse.ArgumentParser(description="Army list JSON ↔ Markdown conversion")
