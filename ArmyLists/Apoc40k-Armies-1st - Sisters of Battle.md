---
faction: Sisters of Battle
source: Apoc40k-Armies-1st - Sisters of Battle.md
---

# Sisters of Battle Army List

## Unit 1 — HQ — Morvenn Vahl

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 8" | 3+ | 3+ | 2 | 3 | 6 | 5+ | 1 | 8 |

### Keywords
Heavy, Vehicle, Walker, Character

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Order of Our Martyred Lady, Morvenn Vahl

### Abilities
```
Righteous Reprimand: Re-roll hit rolls of 1 for attacks made by friendly Adepta Sororitas units whilst they are within 6" of this unit.
Leader of the Faithful: Improve the Save characteristic (to a maximum of 3+) of friendly Adepta Sororitas Infantry units by 1 whilst they are wholly within 6" of this unit.
Morvenn Vahl is a unit that contains 1 model. It is equipped with: Fidelis; Paragon Missile Launcher; Lance of Illumination. You can only include one of this unit in your army.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Fidelis | Small Arms | 36" | User | 6+ | 10+ | Rapid Fire |
| Paragon Missile Launcher (Prioris) | Heavy | 36" | 2 | 5+ | 6+ |  |
| Lance of Illumination | Melee | Melee | User | 6+ | 6+ |  |

## Unit 2 — HQ — Canoness

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 2+ | 2+ | 1 | 1 | 7 | 5+ | 1 | 4 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Canoness

### Abilities
```
Lead the Righteous: Re-roll hit rolls of 1 for attacks made by friendly <Order> units whilst they are within 6" of this unit.
It is equipped with: Master-crafted Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Master-crafted Weapons | Melee | Melee | User | 8+ | 9+ |  |

## Unit 3 — HQ — Missionary

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 4+ | 1 | 1 | 5 | 8+ | 1 | 3 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Ministorum Priest, Missionary

### Abilities
```
War Hymns: Add 1 to the Attacks characteristic of friendly Adeptus Ministorum Infantry and Astra Militarum Infantry units whilst they are making Fight actions whilst within 6" of any friendly units with this ability.
Word of the Emperor: Re-roll failed Morale tests taken for Adeptus Ministorum Infantry units whilst within 6" of any friendly units with this ability.
Lone Mission: No more than one Missionary unit can be included in each Detachment.
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Close Combat Weapons | Melee | Melee | User | 10+ | 10+ |  |

## Unit 4 — HQ — Hospitaller

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 1 | 1 | 6 | 6+ | 1 | 2 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Hospitaller

### Abilities
```
Medicus Ministorum: At the end of the Action phase, this unit can attempt to heal one friendly Adeptus Ministorum Light unit in base contact with it. If it does, roll one D6; on a 4+ remove one damage marker from that Light unit. Only one attempt to heal each unit can be made each turn.
It is equipped with: Chirurgeon's Tools.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Chirurgeon's Tools | Melee | Melee | User | 11+ | 11+ |  |

## Unit 5 — Troops — Battle Sisters Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 1 | 1 | 7 | 6+ | 5 | 3 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Battle Sisters Squad

### Abilities
```
It is equipped with: Boltguns; Close Combat Weapons.
```

### Options
- per: Per Unit
  Pt: 1
  title: Heavy Weapon
  text: Can also be equipped with one of the following
  Choose 1: Heavy Bolter; Heavy Flamer; Multi-melta

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Heavy Flamer | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Multi-melta | Heavy | 24" | 1 | 10+ | 4+ |  |
| Boltguns | Small Arms | 24" | User | 7+ | 9+ | Rapid Fire |
| Close Combat Weapons | Melee | Melee | User | 8+ | 9+ |  |

### Profiles
#### 10 Battle Sisters Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 2 | 2 | 7 | 6+ | 10 | 6 |

## Unit 6 — Troops — Zealots

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 5+ | 5+ | 2 | 4 | 7 | 11+ | 20 | 4 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Zealots

### Abilities
```
It is equipped with: Autopistols; Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Autopistols | Small Arms | 12" | User | 8+ | 10+ |  |
| Close Combat Weapons | Melee | Melee | User | 8+ | 10+ |  |

### Profiles
#### 30 Zealots
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 5+ | 5+ | 3 | 6 | 7 | 11+ | 30 | 6 |

## Unit 7 — Elites — Imagifier

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 1 | 1 | 6 | 6+ | 1 | 2 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Imagifier

### Abilities
```
Simulacrum Imperialis: If any friendly units with this ability are on the battlefield when an Adepta Sororitas Command Asset is played, roll one D12; on a roll of 10+ return that Command Asset to your hand instead of discarding it. That Command Asset cannot be played again this turn.
It is equipped with: Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Close Combat Weapons | Melee | Melee | User | 10+ | 10+ |  |

## Unit 8 — Elites — Celestian Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 1 | 1 | 7 | 6+ | 5 | 4 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Celestian Squad

### Abilities
```
Bodyguard: At the start of the Damage phase, you can select one friendly <Order> Light Character unit that has at least one blast marker next to it and is within 6" of this unit. Remove up to D3 blast markers from that Character unit and place them next to this unit.
Sworn Protectors: You can re-roll hit rolls for attacks made by this unit whilst it is within 6" of any friendly <Order> Canoness units.
It is equipped with: Boltguns; Close Combat Weapons.
```

### Options
- per: Per Unit
  Pt: 1
  title: Heavy Weapon
  text: Can also be equipped with one of the following
  Choose 1: Heavy Bolter; Heavy Flamer; Multi-melta

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Heavy Flamer | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Multi-melta | Heavy | 24" | 1 | 10+ | 4+ |  |
| Boltguns | Small Arms | 24" | User | 7+ | 9+ | Rapid Fire |
| Close Combat Weapons | Melee | Melee | x2 | 8+ | 9+ |  |

### Profiles
#### 10 Celestian Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 2 | 2 | 7 | 6+ | 10 | 8 |

## Unit 9 — Elites — Celestian Sacrosancts

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 4+ | 1 | 1 | 7 | 6+ | 5 | 4 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Celestian Sacrosancts

### Abilities
```
Sacresant Shield: Improve the Save characteristic (to a maximum of 3+) of models in this unit by 1 against attacks made with melee weapons.
Sworn Protectors: You can re-roll hit rolls for attacks made with melee weapons by this unit whilst it is within 6" of any friendly <Order> Canoness units.
It is equipped with: Anointed Halberds.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Anointed Halberds | Melee | Melee | x2 | 6+ | 8+ |  |

### Profiles
#### 10 Celestian Sacrosancts
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 4+ | 2 | 2 | 7 | 6+ | 10 | 8 |

## Unit 10 — Elites — Dialogus

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 1 | 1 | 6 | 11+ | 1 | 1 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Dialogus

### Abilities
```
Laud Hailer: You can re-roll Morale tests taken for friendly Adepta Sororitas units whilst they are within 6" of this unit.
It is equipped with: Dialogus Staff.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Dialogus Staff | Melee | Melee | User | 11+ | 11+ |  |

## Unit 11 — Elites — Preacher

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 4+ | 1 | 1 | 5 | 8+ | 1 | 3 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Ministorum Priest, Preacher

### Abilities
```
War Hymns: Add 1 to the Attacks characteristic of friendly Adeptus Ministorum Infantry and Astra Militarum Infantry units whilst they are making Fight actions whilst within 6" of any friendly units with this ability.
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Close Combat Weapons | Melee | Melee | User | 10+ | 10+ |  |

## Unit 12 — Elites — Repentia Superior

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 1 | 1 | 6 | 5+ | 1 | 3 |

### Keywords
Light, Infantry, Character

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Repentia Superior

### Abilities
```
Driven Onwards: If an <Order> Sisters Repentia unit starts a Move action within 6" of any friendly units with this ability, add 3" to that unit's Move characteristic whilst making that Move action.
Mistress of the Penitent: This unit does not take up slots in a Detachment that contains any <Order> Sisters Repentia units.
It is equipped with: Neural Whips.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Neural Whips | Melee | Melee | User | 8+ | 10+ |  |

## Unit 13 — Elites — Sisters Repentia

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 2 | 1 | 6 | 11+ | 4 | 3 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Sisters Repentia

### Abilities
```
Ignore Damage (6+)
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Penitent Eviscerators.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Penitent Eviscerators | Melee | Melee | User | 5+ | 8+ |  |

### Profiles
#### 9 Sisters Repentia
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 3+ | 4 | 2 | 6 | 11+ | 9 | 6 |

## Unit 14 — Elites — Crusaders

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 4+ | 1 | 1 | 5 | 6+ | 4 | 1 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Ecclesiarchy Battle Conclave, Crusaders

### Abilities
```
Ecclesiarchy Battle Conclave: This unit does not take up slots in a Detachment that contains any Ministorum Priests.
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Crusader Power Swords.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Crusader Power Swords | Melee | Melee | User | 8+ | 8+ |  |

### Profiles
#### 8 Crusaders
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 3+ | 4+ | 2 | 2 | 5 | 6+ | 8 | 2 |

## Unit 15 — Elites — Death Cult Assassins

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 3+ | 4+ | 1 | 1 | 5 | 9+ | 2 | 1 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Ecclesiarchy Battle Conclave, Death Cult Assassins

### Abilities
```
Ecclesiarchy Battle Conclave: This unit does not take up slots in a Detachment that contains any Ministorum Priests.
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Death Cult Power Blades.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Death Cult Power Blades | Melee | Melee | x2 | 8+ | 8+ |  |

### Profiles
#### 4 Death Cult Assassins
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 3+ | 4+ | 2 | 2 | 5 | 9+ | 4 | 2 |

## Unit 16 — Elites — Arco-flagellants

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | - | 1 | 1 | 5 | 11+ | 3 | 2 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Ecclesiarchy Battle Conclave, Arco-flagellants

### Abilities
```
Ignore Damage (6+)
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
Ecclesiarchy Battle Conclave: This unit does not take up slots in a Detachment that contains any Ministorum Priests.
It is equipped with: Arco-flails.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Arco-flails | Melee | Melee | x3 | 7+ | 9+ |  |

### Profiles
#### 5 Arco-flagellants
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | - | 1 | 1 | 5 | 11+ | 5 | 2 |

#### 10 Arco-flagellants
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | - | 2 | 2 | 5 | 11+ | 10 | 3 |

## Unit 17 — Fast — Dominion Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 1 | 1 | 6 | 6+ | 5 | 4 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Dominion Squad

### Abilities
```
Infiltrators
If this unit is not equipped with any Flamers, Meltaguns or Storm Bolters, and/or if it contains 10 models, it is also equipped with Boltguns.
It is equipped with: Flamer; Meltagun; Boltguns; Storm Bolter; Close Combat Weapons.
```

### Options
- per: up to 4
  Pt: 1
  title: Hull Weapon
  text: This unit can also be equipped with up to four of the following in any combination: Flamer; Meltagun; Storm Bolter.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Flamer | Heavy | 8" | 1 | 7+ | 10+ | Inferno |
| Meltagun | Heavy | 12" | 1 | 11+ | 7+ |  |
| Boltguns | Small Arms | 24" | User | 7+ | 9+ | Rapid Fire |
| Storm Bolter | Small Arms | 24" | 1 | 9+ | 10+ | Rapid Fire |
| Close Combat Weapons | Melee | Melee | User | 8+ | 9+ |  |

### Profiles
#### 10 Dominion Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 2 | 2 | 6 | 6+ | 10 | 8 |

## Unit 18 — Fast — Novitiate Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 2 | 2 | 6 | 10+ | 10 | 6 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Novitiate Squad

### Abilities
```
Infiltrators
Novitiate Squad is a unit that contains 10 models. It is equipped with: Novitiate Melee Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Autoguns | Small Arms | 24" | User | 8+ | 10+ | Rapid Fire |
| Novitiate Melee Weapons | Melee | Melee | x2 | 7+ | 9+ |  |

## Unit 19 — Fast — Sanctifiers

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 4+ | 2 | 2 | 6 | 11+ | 10 | 6 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Sanctifiers

### Abilities
```
Infiltrators
Purge the Unclean: Re-roll wound rolls of 1 for attacks made by this unit that target Infantry units.
Sanctifiers is a unit that contains 10 models. It is equipped with: Braziers of Holy Fire; Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Braziers of Holy Fire | Heavy | 12" | 3 | 7+ | 10+ | Inferno |
| Close Combat Weapons | Melee | Melee | User | 7+ | 9+ |  |

## Unit 20 — Fast — Seraphim Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 3+ | 3+ | 1 | 1 | 6 | 6+ | 5 | 5 |

### Keywords
Light, Infantry, Fly, Jump Pack

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Seraphim Squad

### Abilities
```
Deep Strike
It is equipped with: Twin Bolt Pistols; Close Combat Weapons.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Twin Bolt Pistols | Small Arms | 12" | x2 | 7+ | 9+ |  |
| Close Combat Weapons | Melee | Melee | User | 8+ | 9+ |  |

### Profiles
#### 10 Seraphim Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 3+ | 3+ | 2 | 2 | 6 | 6+ | 10 | 9 |

## Unit 21 — Fast — Zephyrim Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 3+ | 3+ | 1 | 1 | 7 | 6+ | 5 | 5 |

### Keywords
Light, Infantry, Jump Pack, Fly

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Zephyrim Squad

### Abilities
```
Deep Strike
It is equipped with: Bolt Pistols; Zephyrim Power Swords.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Bolt Pistols | Small Arms | 12" | User | 7+ | 9+ |  |
| Zephyrim Power Swords | Melee | Melee | x2 | 5+ | 9+ |  |

### Profiles
#### 10 Zephyrim Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 3+ | 3+ | 2 | 2 | 7 | 6+ | 10 | 10 |

## Unit 22 — Heavy — Retributor Squad

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 1 | 1 | 6 | 6+ | 5 | 3 |

### Keywords
Light, Infantry

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Retributor Squad

### Abilities
```
If this unit is not equipped with any Heavy Bolters, Heavy Flamers or Multi-meltas, and/or if it contains 10 models, it is also equipped with Boltguns.
It is equipped with: Heavy Bolter; Heavy Flamer; Multi-melta; Boltguns; Close Combat Weapons.
```

### Options
- per: up to 4
  Pt: 1
  title: Heavy Weapon
  text: This unit can also be equipped with up to four of the following in any combination: Heavy Bolter; Heavy Flamer; Multi-melta.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Heavy Flamer | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Multi-melta | Heavy | 24" | 1 | 10+ | 4+ |  |
| Boltguns | Small Arms | 24" | User | 7+ | 9+ | Rapid Fire |
| Close Combat Weapons | Melee | Melee | User | 8+ | 9+ |  |

### Profiles
#### 10 Retributor Squad
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 6" | 4+ | 3+ | 2 | 2 | 6 | 6+ | 10 | 6 |

## Unit 23 — Heavy — Exorcist

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 5 | 6+ | 1 | 7 |

### Keywords
Heavy, Vehicle

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Exorcist

### Abilities
```
It is equipped with: Exorcist Missile Launcher; Armoured Tracks.
```

### Options
- per: Per Unit
  text: Instead of 1 Exorcist Missile Launcher, this unit can be equipped with Exorcist Conflagration Rockets.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Exorcist Conflagration Rockets | Heavy | 48" | 3 | 6+ | 9+ |  |
| Exorcist Missile Launcher | Heavy | 48" | 2 | 8+ | 5+ |  |
| Armoured Tracks | Melee | Melee | User | 10+ | 10+ |  |

## Unit 24 — Heavy — Immolator

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 6 | 6+ | 1 | 5 |

### Keywords
Heavy, Vehicle, Transport

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Immolator

### Abilities
```
TRANSPORT: This unit can transport up to 6 friendly Adeptus Ministorum Infantry models. It cannot transport Jump Pack models and can only transport Adepta Sororitas models if they have the <Order>, Dialogus or Hospitaller keyword.
It is equipped with: Immolation Flamer; Armoured Tracks.
```

### Options
- per: Per Unit
  text: Instead of 1 Immolation Flamer, this unit can be
  Choose 1: 1 Twin Heavy Bolter; 1 Twin Multi-melta

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Immolation Flamer | Heavy | 12" | 2 | 6+ | 9+ | Inferno |
| Twin Heavy Bolter | Heavy | 36" | 2 | 7+ | 9+ |  |
| Twin Multi-melta | Heavy | 24" | 2 | 10+ | 4+ |  |
| Armoured Tracks | Melee | Melee | User | 10+ | 10+ |  |

## Unit 25 — Heavy — Mortifiers

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 9" | 3+ | 3+ | 1 | 1 | 6 | 7+ | 1 | 5 |

### Keywords
Heavy, Vehicle

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Mortifiers

### Abilities
```
Ignore Damage (6+)
It is equipped with: Heavy Bolter; Heavy Flamer; Mortifier Weapons.
```

### Options
- per: Per Model
  title: Heavy Weapon
  text: For each model this unit contains, it
  Choose 1: 2 Heavy Bolters; 2 Heavy Flamers; 1 Heavy Bolter and 1 Heavy Flamer

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Heavy Flamer | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Mortifier Weapons | Melee | Melee | x2 | 5+ | 7+ |  |

### Profiles
#### 2 Mortifiers
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 9" | 3+ | 3+ | 2 | 2 | 6 | 7+ | 2 | 8 |

#### 3 Mortifiers
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 9" | 3+ | 3+ | 3 | 3 | 6 | 7+ | 3 | 12 |

#### 4 Mortifiers
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 9" | 3+ | 3+ | 4 | 4 | 6 | 7+ | 4 | 16 |

## Unit 26 — Heavy — Penitent Engines

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 4+ | 5+ | 1 | 1 | 6 | 7+ | 1 | 4 |

### Keywords
Heavy, Vehicle

### Traits
Imperium, Adeptus Ministorum, Penitent Engines

### Abilities
```
Ignore Damage (6+)
Zealot: You can re-roll hit rolls for attacks made with melee weapons by this unit.
It is equipped with: Heavy Flamer; Penitent Engine Weapons.
```

### Options
- per: Per Model
  title: Heavy Weapon
  text: For each model this unit contains, it must also be equipped with 2 Heavy Flamers.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Flamer | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Penitent Engine Weapons | Melee | Melee | x2 | 5+ | 7+ |  |

### Profiles
#### 2 Penitent Engines
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 4+ | 5+ | 2 | 2 | 6 | 7+ | 2 | 8 |

#### 3 Penitent Engines
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 4+ | 5+ | 3 | 3 | 6 | 7+ | 3 | 12 |

#### 4 Penitent Engines
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7" | 4+ | 5+ | 4 | 4 | 6 | 7+ | 4 | 16 |

## Unit 27 — Heavy — Castigator

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 5 | 6+ | 1 | 6 |

### Keywords
Heavy, Vehicle

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Castigator

### Abilities
```
A Castigator is a unit that contains 1 model. It is equipped with: Castigator Autocannons; 3 Heavy Bolters; Armoured Tracks.
```

### Options
- per: Per Unit
  text: Instead of 1 Castigator Autocannons, this unit can be equipped with 1 Castigator Battle Cannon.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Castigator Autocannons | Heavy | 48" | 2 | 5+ | 6+ | Rapid Fire |
| Castigator Battle Cannon | Heavy | 48" | 1 | 6+ | 7+ |  |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Armoured Tracks | Melee | Melee | User | 10+ | 10+ |  |

## Unit 28 — Heavy — Paragon Warsuits

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 8" | 3+ | 3+ | 3 | 3 | 6 | 5+ | 3 | 9 |

### Keywords
Heavy, Vehicle, Walker

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Paragon Warsuits

### Abilities
```
Righteous Paragons: Re-roll hit rolls of 1 for attacks made by this unit that target Heavy or Super-heavy units.
Paragon Warsuits is a unit that contains 3 models. Every model is equipped with: Heavy Bolter; Paragon Storm Bolters; Paragon War Blade.
```

### Options
- per: Per model
  Pt: 1
  text: Any model can have its Heavy Bolter replaced with 1 Multi-melta.

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Heavy Bolter | Heavy | 36" | 1 | 7+ | 9+ |  |
| Paragon Storm Bolters | Small Arms | 24" | User | 7+ | 10+ | Rapid Fire |
| Multi-melta | Heavy | 24" | 1 | 10+ | 4+ |  |
| Paragon War Blade | Melee | Melee | User | 6+ | 6+ |  |

## Unit 29 — Transport — Immolator

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 6 | 6+ | 1 | 5 |

### Keywords
Heavy, Vehicle, Transport

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Immolator

### Abilities
```
TRANSPORT: This unit can transport up to 6 friendly Adeptus Ministorum Infantry models. It cannot transport Jump Pack models and can only transport Adepta Sororitas models if they have the <Order>, Dialogus or Hospitaller keyword.
It is equipped with: Immolation Flamer; Armoured Tracks.
```

### Options
- per: Per Unit
  text: Instead of 1 Immolation Flamer, this unit can be
  Choose 1: 1 Twin Heavy Bolter; 1 Twin Multi-melta

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Immolation Flamer | Heavy | 12" | 2 | 6+ | 9+ | Inferno |
| Twin Heavy Bolter | Heavy | 36" | 2 | 7+ | 9+ |  |
| Twin Multi-melta | Heavy | 24" | 2 | 10+ | 4+ |  |
| Armoured Tracks | Melee | Melee | User | 10+ | 10+ |  |

## Unit 30 — Transport — Sororitas Rhino

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 6 | 6+ | 1 | 5 |

### Keywords
Heavy, Vehicle, Transport

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Rhino, Sororitas Rhino

### Abilities
```
TRANSPORT: This unit can transport up to 10 friendly Adeptus Ministorum Infantry models. It cannot transport Jump Pack models and can only transport Adepta Sororitas models if they have the <Order>, Dialogus or Hospitaller keyword.
It is equipped with: Storm Bolter; Armoured Tracks.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Storm Bolter | Small Arms | 24" | 1 | 9+ | 10+ | Rapid Fire |
| Armoured Tracks | Melee | Melee | User | 10+ | 10+ |  |

## Unit 31 — Transport — Repressor

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 6+ | 3+ | 1 | 2 | 6 | 6+ | 1 | 7 |

### Keywords
Heavy, Vehicle, Transport

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, <Order>, Repressor

### Abilities
```
TRANSPORT: This unit can transport up to 10 friendly Adeptus Ministorum Infantry models. It cannot transport Jump Pack models and can only transport Adepta Sororitas models if they have the <Order>, Dialogus or Hospitaller keyword.
A Repressor is a unit that contains 1 model. It is equipped with: Twin Heavy Flamer; Storm Bolter; Dozer Ram.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| Twin Heavy Flamer | Heavy | 8" | 2 | 7+ | 10+ | Inferno |
| Storm Bolter | Small Arms | 24" | 1 | 9+ | 10+ | Rapid Fire |
| Dozer Ram | Melee | Melee | User | 9+ | 10+ |  |

## Unit 32 — Lord — Celestine & Geminae

### Stats
| M | WS | BS | A | W | Ld | Sv | N | Pt |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 12" | 2+ | 2+ | 3 | 4 | 7 | 4+ | 3 | 14 |

### Keywords
Light, Infantry, Fly, Character, Jump Pack

### Profile Keywords
Infantry

### Traits
Imperium, Adeptus Ministorum, Adepta Sororitas, Celestine, Geminae Superia

### Abilities
```
Beacon of Faith: Improve the Save characteristic (to a maximum of 3+) of friendly Adepta Sororitas units by 1 whilst they are wholly within 6" of this unit.
Miraculous Intervention: The first time the number of damage markers next to this unit equals its Wounds characteristic, roll a D6; on a 2+ this unit is not destroyed, and one damage marker is removed from it.
Lifewards: At the start of the Damage phase, Remove up to D3 blast markers from this Character.
Celestine & Geminae is a unit that contains 3 models: 1 Celestine and 2 Geminae Superia. You can only include one of this unit in your army.
It is equipped with: The Ardent Blade (Ranged); Ardent Blades.
```

### Weapons
| Name | Type | Range | A | SAP | SAT | Abilities |
| --- | --- | --- | --- | --- | --- | --- |
| The Ardent Blade (Ranged) | Heavy | 8" | 1 | 6+ | 9+ | Inferno |
| Ardent Blades | Melee | Melee | User | 7+ | 7+ |  |
