...
} from '../utils/handStorage.js'

const RESET_CONFIRM_MS = 2500

export default function HandModal({ army, onClose }) {
  const [handState, setHandState] = useState(() => loadHandState(army.id, army.cards ?? []))
  const [undoStack, setUndoStack] = useState([])
  const [activePile, setActivePile] = useState('hand')
  ...
  function applyAction(updater) {
    setHandState((current) => {
      ...
      setUndoStack((stack) =>
        [...stack, cloneHandState(current)].slice(-MAX_HAND_UNDO),
      )
      return next
    })
  }
                      ...
                      headerAction={
                        activePile === 'hand' ? (
                          <button
                            type="button"
                            className="secondary-btn"
                            onClick={() => applyAction((state) => discardFromHand(state, entryId))}
                          >
                            Discard
                          </button>
                        ) : (
